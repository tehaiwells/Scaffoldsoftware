...
// ---- Alerts (al-*): small pictures for the alert rows, the bell, the empty state and the drawer's scene. Built from the main sprites plus a badge, drawn once each as blob images. ----
const AL_SHADOW='<ellipse cx="38" cy="64" rx="27" ry="5" fill="#2a1d08" fill-opacity=".15"/>';
const alOct=(cx,cy,r)=>{let d='';for(let i=0;i<8;i++){const a=Math.PI/8+i*Math.PI/4;d+=(i?'L':'M')+(+(cx+r*Math.cos(a)).toFixed(1))+' '+(+(cy+r*Math.sin(a)).toFixed(1));}return d+'Z';};
const alClock=(c,ring)=>'<circle cx="58" cy="49" r="14" fill="#fff" stroke="'+ring+'" stroke-width="3"/><path d="M58 41.5v8l5.5 3.6" fill="none" stroke="'+c+'" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/>';
const alBoard='<rect x="12" y="10" width="42" height="52" rx="5" fill="#c9a36a" stroke="#6b4a2b" stroke-width="2"/><rect x="17" y="16" width="32" height="42" rx="2" fill="#fbfaf4"/><rect x="24" y="6" width="18" height="9" rx="2.5" fill="#7c8a86" stroke="#44504f" stroke-width="1.5"/><path d="M22 26h20M22 33h15M22 40h18M22 47h11" stroke="#c9d1c2" stroke-width="2.4" stroke-linecap="round"/>';
const AL_ART={
 'al-bell':'<ellipse cx="40" cy="65" rx="22" ry="4.5" fill="#2a1d08" fill-opacity=".16"/><path d="M11 22c-3 5-4 11-2.5 17M69 22c3 5 4 11 2.5 17M5 17c-5 8-6 18-3.5 26M75 17c5 8 6 18 3.5 26" fill="none" stroke="#e0563a" stroke-width="3" stroke-linecap="round"/><circle cx="40" cy="8" r="4" fill="#6f5a12"/><path d="M40 10c-12 0-19.5 8.6-19.5 20.4v11.4l-6.1 9.6c-1.2 2 .2 4.1 2.6 4.1h46c2.4 0 3.8-2.1 2.6-4.1l-6.1-9.6V30.4C59.5 18.6 52 10 40 10Z" fill="#f2c434" stroke="#6f5a12" stroke-width="2.4" stroke-linejoin="round"/><path d="M28 31c0-7.6 4.4-13 10.6-14.2" fill="none" stroke="#fff6c4" stroke-width="4" stroke-linecap="round" opacity=".85"/><path d="M52 44v7" stroke="#d09c14" stroke-width="3.5" stroke-linecap="round" opacity=".7"/><circle cx="40" cy="61.5" r="5.5" fill="#6f5a12"/>',
 'al-low':AL_SHADOW+'<use href="#spr-stillage" x="2" y="16" width="66" height="44"/><circle cx="60" cy="18" r="13" fill="#e0563a" stroke="#fff" stroke-width="2.5"/><path d="M60 11v13M54.5 19l5.5 6 5.5-6" fill="none" stroke="#fff" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/>',
 'al-list':AL_SHADOW+alBoard+alClock('#b87a0c','#e8a93a'),
 'al-late':AL_SHADOW+alBoard+alClock('#b5462f','#e0563a')+'<circle cx="70" cy="37" r="6" fill="#e0563a" stroke="#fff" stroke-width="2"/><path d="M70 34v3.4" stroke="#fff" stroke-width="2" stroke-linecap="round"/><circle cx="70" cy="40" r="1" fill="#fff"/>',
 'al-blocked':AL_SHADOW+'<use href="#spr-forklift-load" x="0" y="10" width="66" height="53"/><path d="'+alOct(60,18,13)+'" fill="#d64a33" stroke="#fff" stroke-width="2.5" stroke-linejoin="round"/><path d="M53 18h14" stroke="#fff" stroke-width="3.6" stroke-linecap="round"/>',
 'al-damaged':AL_SHADOW+'<use href="#spr-stillage" x="2" y="16" width="66" height="44"/><path d="M10 50 58 26" stroke="#f0b429" stroke-width="6" stroke-linecap="round" opacity=".92"/><path d="M14 48l4-2M24 43l4-2M34 38l4-2M44 33l4-2" stroke="#2f2a1c" stroke-width="3.2" stroke-linecap="butt"/><path d="M60 4 74 29H46Z" fill="#f0b429" stroke="#6b4a10" stroke-width="2.2" stroke-linejoin="round"/><path d="M60 12v8.5" stroke="#3a2a05" stroke-width="3.2" stroke-linecap="round"/><circle cx="60" cy="25" r="1.9" fill="#3a2a05"/>',
 'al-clash':AL_SHADOW+'<use href="#spr-truck12" x="-2" y="20" width="72" height="39"/><rect x="45" y="4" width="30" height="28" rx="4" fill="#fff" stroke="#b5462f" stroke-width="2"/><path d="M45 8a4 4 0 0 1 4-4h22a4 4 0 0 1 4 4v5H45Z" fill="#e0563a"/><path d="M52 2v5M68 2v5" stroke="#6b2a1c" stroke-width="2.2" stroke-linecap="round"/><text x="60" y="28.5" text-anchor="middle" font-family="Arial,sans-serif" font-size="13" font-weight="800" fill="#b5462f">2&#215;</text>',
 'al-clear':'<ellipse cx="46" cy="66" rx="36" ry="6" fill="#2a1d08" fill-opacity=".13"/><use href="#spr-stillage" x="4" y="30" width="58" height="39"/><use href="#spr-stillage" x="4" y="12" width="58" height="39"/><use href="#spr-cage" x="46" y="36" width="46" height="31"/><circle cx="74" cy="18" r="14" fill="#6fae2f" stroke="#fff" stroke-width="3"/><path d="M67 18.5l5 5 9-10" fill="none" stroke="#fff" stroke-width="3.4" stroke-linecap="round" stroke-linejoin="round"/>',
 'al-scene':'<path d="M-20 104 160 24 360 104 180 184Z" fill="#8fa866"/><path d="M-20 140 236 26h36L-20 158Z" fill="#55595b" opacity=".85"/><path d="M40 106 162 52 300 106 178 160Z" fill="#d8d1c1"/><path d="M40 106 178 160 300 106v5L178 165 40 111Z" fill="#a79f8b"/><path d="M58 106 162 60 282 106 178 152Z" fill="none" stroke="#e3bd2c" stroke-width="1.6" opacity=".9"/>'
  +'<use href="#spr-tree" x="290" y="40" width="40" height="47"/><use href="#spr-tree" x="148" y="0" width="34" height="40"/>'
  +'<use href="#spr-stillage" x="104" y="58" width="60" height="40"/><use href="#spr-stillage" x="104" y="40" width="60" height="40"/><use href="#spr-stillage" x="150" y="80" width="60" height="40"/><use href="#spr-cage" x="80" y="92" width="46" height="31"/><use href="#spr-forklift-load" x="196" y="76" width="70" height="55"/><use href="#spr-worker-busy" x="150" y="116" width="18" height="34"/>'
  +'<path d="M232 44V92" stroke="#56636a" stroke-width="4" stroke-linecap="round"/><rect x="225" y="90" width="14" height="5" rx="1.5" fill="#44504f"/><path d="M224 44a8 8 0 0 1 16 0v4h-16Z" fill="#f0a82a" stroke="#8a5a0a" stroke-width="1.5"/><rect x="222" y="47" width="20" height="4" rx="1.5" fill="#56636a"/><path d="M216 34l-10-6M248 34l10-6M232 30V18" stroke="#ffd35a" stroke-width="3" stroke-linecap="round" opacity=".9"/>'};
const AL_BOX={'al-scene':'0 0 320 170','al-clear':'0 0 96 74'};
export const alImg=(key,cls='')=>ovImg(key,'al-img'+(cls?' '+cls:''),AL_BOX[key]??'0 0 80 72',AL_ART[key]??AL_ART['al-bell']);

// ----- Reports (hc-): the report board in the Reports hero (symbol hc-board, 170 x 184: a standing whiteboard turned the way the yard sprites face, with
// bars in the yard / site / truck colours and a trend line) and the nav icon. The board face is a sheared plane: hcP(u,v) maps 0..1 across and down it.
const hcP=(u,v)=>[18+u*132+v*0,62-u*62+v*84],hcN=p=>p.map(n=>+n.toFixed(1)).join(' '),hcQ=pts=>'M'+pts.map(hcN).join('L')+'Z';
function hcBoardArt(){
 const leg=(u,x)=>{const [a,b]=hcP(u,1);return '<path d="M'+hcN([a,b-4])+'v'+(34+x)+'" stroke="#3d494e" stroke-width="5" stroke-linecap="round"/><path d="M'+hcN([a-7,b+30+x+3])+'h14" stroke="#2c3438" stroke-width="4" stroke-linecap="round"/>';};
 let s='<ellipse cx="86" cy="164" rx="74" ry="12" fill="#2a1d08" fill-opacity=".16"/>'+leg(.12,0)+leg(.88,0);
 s+='<path d="'+hcQ([hcP(1,0),[154,2],[154,86],hcP(1,1)])+'" fill="#1f4a33"/><path d="'+hcQ([hcP(0,0),hcP(1,0),[154,2],[22,64]])+'" fill="#3e7a52"/>';
 s+='<path d="'+hcQ([hcP(0,0),hcP(1,0),hcP(1,1),hcP(0,1)])+'" fill="#2b5a3d"/><path d="'+hcQ([hcP(.035,.06),hcP(.965,.06),hcP(.965,.94),hcP(.035,.94)])+'" fill="#fbfaf4"/>';
 s+='<path d="M'+hcN(hcP(.09,.15))+'L'+hcN(hcP(.46,.15))+'" stroke="#6fae2f" stroke-width="3.2" stroke-linecap="round"/><path d="M'+hcN(hcP(.09,.25))+'L'+hcN(hcP(.32,.25))+'" stroke="#c9d1c2" stroke-width="2.2" stroke-linecap="round"/>';
 s+='<path d="M'+hcN(hcP(.08,.86))+'L'+hcN(hcP(.93,.86))+'" stroke="#c9d1c2" stroke-width="1.4"/>';
 const bars=[[.12,.3,'#7fb536'],[.3,.46,'#4d8fc4'],[.48,.38,'#7fb536'],[.66,.56,'#e8a52a']];
 for(const [u,h,c] of bars)s+='<path d="'+hcQ([hcP(u,.86),hcP(u+.13,.86),hcP(u+.13,.86-h),hcP(u,.86-h)])+'" fill="'+c+'"/>';
 const line=[[.14,.46],[.36,.36],[.55,.42],[.74,.24],[.9,.16]];
 s+='<path d="M'+line.map(([u,v])=>hcN(hcP(u,v))).join('L')+'" fill="none" stroke="#d65f37" stroke-width="2.6" stroke-linejoin="round" stroke-linecap="round"/>'+line.map(([u,v])=>{const [x,y]=hcP(u,v);return '<circle cx="'+x.toFixed(1)+'" cy="'+y.toFixed(1)+'" r="3.2" fill="#fff" stroke="#d65f37" stroke-width="2"/>';}).join('');
 return s;}
let hcSheetHTML=null;
...
export const tdIcon=()=>'<svg class="line-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M3 17h18M7.5 17a4.5 4.5 0 0 1 9 0M12 5.5v3M5.4 9.4l2 2M18.6 9.4l-2 2M2.8 13.6h2.4M18.8 13.6h2.4M8 20.5h8"/></svg>';// Today: the sun coming up (Schedule keeps the calendar)
