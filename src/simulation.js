...
const operational=['rotate','workerSkills','createJob','assignJob','nextJob','takeOffJob','cancelJob','jobsMode','siteDetails','fixtures','commitLayout','cancelLayout','siteBoundary','retire','scrapContainer','quickAdjust','allocateLoadList','workerCommand','parking','yard','container','containerSettings','product','override','seed','importCatalogue','site','archive','truck','resources','queue','allocate','cancel','retry','dispatch','unload','condition','pause','count','observe','cancelCount'];
export class Simulation {
  ...
  execute(action,input,key){
    ...
    return atomic(this.db,()=>{const previous=cached(this.db,'SELECT * FROM commands WHERE company_id=? AND key=?').get(this.user.company_id,key);if(previous){requireRule(previous.fingerprint===fingerprint,'This idempotency key was used for a different action.');return JSON.parse(previous.result);}const result=this[action](input);this.repo.event(this.user.id,'COMMAND',{reason:action,key});cached(this.db,'INSERT INTO commands VALUES(?,?,?,?)').run(this.user.company_id,key,fingerprint,JSON.stringify(result??{ok:true}));return result;});
  }
...
Object.assign(Simulation.prototype,catalogueMethods,inventoryMethods,logisticsMethods,movementMethods,workerMethods,forkliftMethods,materialsMethods,fleetMethods,layoutMethods,jobMethods,turningMethods);
