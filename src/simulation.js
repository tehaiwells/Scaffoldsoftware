...
export class Simulation {
  ...
  execute(action,input,key){
    ...
    else if(['bdSaveDetails','bdSaveLogo','bdRemoveLogo'].includes(action))this.auth.require(this.user,'company.manage');
    else if(['request','returnStock','cancelRequest','createLoadList','cancelLoadList','reschedule','requestCollection','cancelCollection'].includes(action))this.auth.require(this.user,'requests.create');
    else if(['hireRate','hireSiteRate'].includes(action)){if(!this.auth.permissions(this.user).includes('finance.view'))this.auth.require(this.user,'company.manage');}
  ...
  buildSnapshot(page,opts){
    ...
    result.alerts=this.alertsView(result,{allContainers,allBalances,operations,cal});this.rtAlerts(result);
  ...
  scheduleSnapshot(result,cal,allTrucks,products){
    ...
    result.collections=this.rtViews(cal,ctx);
    const items=[...result.loadLists,...result.requests.filter(r=>!r.loadList),...result.collections],byId=new Map(items.map(x=>[x.id,x])),clashes=this.clashIndex(perms.includes('operations.manage')?[...items,...result.requests.filter(r=>r.loadList)]:undefined);
...
Object.assign(Simulation.prototype,catalogueMethods,inventoryMethods,logisticsMethods,movementMethods,workerMethods,forkliftMethods,materialsMethods,fleetMethods,layoutMethods,jobMethods,turningMethods,scheduleMethods,alertMethods,reportsMethods,hireMethods,collectionMethods,brandMethods);
