...
export const workerMethods={
  forkliftPosition(machine){
    ...
    const loc=this.repo.get(machine.location),obstacles=[...this.occupied(loc.id).map(c=>rect(c)),{...loc.loading,w:2000,h:1500},...this.fixtureObstacles(loc),...this.repo.all('resource').filter(r=>r.enabled&&r.location===loc.id&&r.id!==machine.id&&Number.isFinite(r.x)&&Number.isFinite(r.y)).map(r=>r.type==='FORKLIFT'?{x:r.x,y:r.y,...this.forkliftShape(r)}:{x:r.x,y:r.y,w:500,h:500})];
  ...
  workerPosition(worker){
    ...
    const loc=this.repo.get(worker.location),obstacles=[...this.occupied(loc.id).map(c=>rect(c)),...(loc.loading?[{...loc.loading,w:2000,h:1500}]:[]),...this.fixtureObstacles(loc),...this.repo.all('resource').filter(r=>r.enabled&&r.location===loc.id&&r.id!==worker.id&&Number.isFinite(r.x)&&Number.isFinite(r.y)).map(r=>r.type==='FORKLIFT'?{x:r.x,y:r.y,...this.forkliftShape(r)}:{x:r.x,y:r.y,w:500,h:500})];
  ...
  workerCommand(input){
    ...
    requireRule(['MOVE','STOP','HOLD','AUTO','MOUNT','DISMOUNT','PICKUP','PLACE'].includes(input.order),'Choose a worker command.');requireRule(input.resume===undefined||typeof input.resume==='boolean','Choose resume on or off.');if(input.order==='MOVE'&&input.target!==undefined){const s=this.resolveTarget(worker,input.target);input={...input,x:s.x,y:s.y};}
    if(input.order==='DISMOUNT'){
      ...
      machine.driver=null;machine.claimedBy=null;this.repo.save(machine);worker.mountedOn=null;worker.workerMode=input.resume===true?'AUTO':'HOLD';if(input.resume===true)worker.dwellUntil=new Date(Date.now()+30000).toISOString();Object.assign(worker,target);return this.repo.save(worker);
    }
    ...
    if(input.order==='MOUNT'){
      ...
      if(worker.job)this.releaseJob(worker,'Order from the yard office');machine.claimedBy=worker.id;this.repo.save(machine);worker.mountTarget=machine.id;worker.walk={path,next:1};worker.workerMode='MOVING';
      ...
      requireRule(path,'No clear walking route. Choose open ground inside the yard.');if(worker.job)this.releaseJob(worker,'Order from the yard office');worker.walk={path,next:1,resume:input.resume===true};worker.workerMode='MOVING';
    }else{if(input.order==='AUTO'){worker.assignHoldUntil=null;worker.dwellUntil=null;worker.jobSkipUntil=null;}else if(worker.job)this.releaseJob(worker,'Order from the yard office');worker.workerMode=input.order==='AUTO'?'AUTO':'HOLD';}
