...
export const materialsMethods={
  ...
  stockLog(kind,limit=100){integer(limit,'Page size',1,1000);const events=LIST_EVENTS[kind];requireRule(events,'Choose additions or removals.');const scoped=this.auth.permissions(this.user).includes('operations.manage');const rows=this.db.prepare(`SELECT * FROM ledger WHERE company_id=? AND event IN (${events.map(()=>'?').join(',')}) ORDER BY sequence DESC LIMIT ?`).all(this.user.company_id,...events,limit);return scoped?rows:rows.filter(l=>{try{this.assertSite(l.source??l.destination);return true;}catch{return false;}});},
