...
export const materialsMethods={
  ...
  register(balances,products){
    ...
    return [...rows.values()].map(r=>{const p=products.find(p=>p.id===r.product);return {...r,name:p?.name??'Unknown product',system:p?.system??null,category:p?.category??null,unitWeight:p?.unitWeight??null,available:r.quantity-r.reserved};}).sort((a,b)=>a.name.localeCompare(b.name));
  },
  stockLog(kind,limit=100){integer(limit,'Page size',1,1000);const events=LIST_EVENTS[kind];requireRule(events,'Choose additions or removals.');const scoped=this.auth.permissions(this.user).includes('operations.manage');const retired=new Set(this.repo.all('product').filter(p=>p.retired).map(p=>p.id));const rows=this.db.prepare(`SELECT * FROM ledger WHERE company_id=? AND event IN (${events.map(()=>'?').join(',')}) ORDER BY sequence DESC LIMIT ?`).all(this.user.company_id,...events,limit).filter(l=>!retired.has(l.product_id));return scoped?rows:rows.filter(l=>{try{this.assertSite(l.source??l.destination);return true;}catch{return false;}});},
  ...
  loadLists(){const operations=this.auth.permissions(this.user).includes('operations.manage');return this.repo.all('loadList').filter(l=>{if(operations)return true;try{this.assertSite(l.site);return true;}catch{return false;}}).map(l=>this.loadListView(l));},
