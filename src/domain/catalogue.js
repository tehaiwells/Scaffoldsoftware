...
const withSettings=(p,setting,pack)=>({...p,unitWeight:setting?.unitWeight??p.unitWeight,packQuantity:setting?.packQuantity??pack?.operatingQuantity??null,...(setting?{figuresStatus:setting.status??'COMPANY CONFIGURED',figuresSource:sourceOf(setting)}:{})});
...
export const catalogueMethods={
  ...
  override(input){const product=this.repo.get(input.product,'product');const settings=this.repo.all('productSettings').find(s=>s.product===product.id);const before=this.effective(product.id);
