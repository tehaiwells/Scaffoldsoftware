...
export const movementMethods={
  advance(task){
    ...
    if(task.state==='PICKING'){
      ...
      if(task.type==='REPACK'){
        const source=this.repo.get(task.sourceContainer,'container');requireRule(source.location===task.from,'Partial-pick source moved.');this.assertCountFree(source);this.repo.balance(source.id,task.product,-task.quantity);this.repo.balance(c.id,task.product,task.quantity);this.repo.event(task.actor,'REPACK_PICKUP',{container:c.id,product:task.product,quantity:task.quantity,source:source.id,destination:task.machine,task:task.id,request:task.request,key:task.id+':pickup'});
